...
<div class="dashboard">
  <div class="container">
    ...
    <div class="welcome-section animate-fade-in">
      <h1 class="greeting">{greeting}, {$auth.user?.name?.split(' ')[0]}! 👋</h1>
    ...
    <!-- Attendance Button -->
    <div class="attendance-section animate-fade-in" style="animation-delay: 200ms">
      <AttendanceButton />
    </div>
          ...
          <span class="stat-value">
            {#if $attendance.today?.work_hours}
              ...
              {#if parseFloat($attendance.today.overtime_hours) > 0}
                <span class="overtime-tag">+{$attendance.today.overtime_hours} OT</span>
              {/if}
            {:else}
              -
            {/if}
    ...
    <!-- Schedule Info -->
    {#if schedule}
      <div class="schedule-section animate-fade-in" style="animation-delay: 500ms">
        <h3>📆 Waktu Bekerja</h3>
        <div class="schedule-grid">
          ...
          <div class="schedule-item">
            ...
            <span class="schedule-value">{schedule.checkIn.earliest} - {schedule.checkIn.latest}</span>
          </div>
          <div class="schedule-item">
            <span class="schedule-label">Check-Out (Ahad-Rabu)</span>
            <span class="schedule-value">{schedule.checkOut.regular.earliest} - {schedule.checkOut.regular.latest}</span>
          </div>
          <div class="schedule-item">
            <span class="schedule-label">Check-Out (Khamis)</span>
            <span class="schedule-value">{schedule.checkOut.thursday.earliest} - {schedule.checkOut.thursday.latest}</span>
          </div>
        </div>
      </div>
    {/if}
